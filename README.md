...
- **Dashboard** — check-ins due, upcoming birthdays, plans, gift ideas, quiet relationships, recent activity, quick add / voice note.
- **People** — searchable/filterable list; rich profiles with relationship health, interests, dislikes, personality notes, gifts, important dates, reminders, and a timeline.
...
- **Voice notes** — record → on-device transcription (Speech framework, mock fallback in the Simulator) → mock AI extraction → review screen → applies interests, gifts, reminders, dates, personality notes, and logs a timeline interaction.
- **AI** — `AIService` protocol with `MockAIService` (keyword heuristics, fully offline) and an `LLMAIService` stub for plugging in a real model later. No AI logic in views.
- **Search** — local search across everything, with natural-ish queries: *"who likes F1?"*, *"who did I talk to about internships?"*, *"birthdays in November"*.
...
- **Backup** — JSON export via the share sheet; import merges by name.
- Seed data (family, close friend, roommate, classmate, mentor, acquaintance, professional contact) loads on first launch and can be wiped from Settings.
...
SocialClimber/
  ...
  Services/      RelationshipHealth, AIService (+Mock/LLM stub),
