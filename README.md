...
- **AI**: `AIService` protocol with `MockAIService` (keyword heuristics, fully offline) and a hosted provider for structured JSON extraction. Extraction also infers how you know each contact from what the conversation shows, so an automatically-created person doesn't sit on the generic "Acquaintance" default; a category you picked yourself is never overwritten. API keys are stored only in iOS Keychain.
...
- **Timeline**: one tab for time in both directions. *Upcoming* is a merged 60-day feed of birthdays, dates, reminders, and (optional, read-only) Google Calendar events that mention known people; swipe to track one as a planned hangout. *Past* is the record of what has actually happened.
- **Past Events**: significant things that actually happened, to you and to the people you track, read out of the conversations you capture and import. Only completed changes of state are stored (a school decision, a job, a move, a breakup, a loss, a falling-out); plans, topics, and banter are not events, and the same bar applies to what a real AI provider returns. An event is kept only when it attributes to you or to exactly one contact. Filterable by kind and by who it happened to, with a section on each person's profile.
...
- **Instagram via Google Drive**: set Instagram's JSON "Download your information" export to deliver to Google Drive, connect Drive once in Settings, then tap Sync on the Home screen (or on Social Health). Both Meta's expanded folder tree and zip delivery formats are supported. New DMs become reviewable Instagram interactions on people's timelines (matched by Instagram username, contact method, or name), and everything is parsed on-device with the raw export deleted immediately. Follower and following counts are deliberately not tracked: Meta's monthly exports are date-limited slices rather than snapshots, so nothing honest can be said about who followed or unfollowed you. Same bring-your-own OAuth client as Calendar (enable the Drive API on the same project). An optional daily local notification reminds you to sync, since iOS won't run it in the background reliably.
- **Social Health**: a transparent 0-100 aggregate of your whole social life (from the Dashboard): average relationship score, interaction momentum, breadth of people reached, and relationships going quiet, with every point attributed to a labeled factor and the relationships pulling the score down listed out. Every term is a continuous curve rather than a bucket or a cap, so the number responds to a day passing or a conversation landing instead of sitting on the same integer for weeks.
...
For **Brief** (the morning-briefing app), Social Climber also writes a richer, one-way feed, `socialclimber_brief_feed_v1.json`, every time the dashboard refreshes: up to 3 recent days of human-readable activity lines (interactions, events, new people, captures, and anything significant that happened to you or someone you track) plus today's social reminders (follow-ups, birthdays and important dates, upcoming events, check-in nudges). Unlike the peer-bridge snapshot, this feed is written for your own eyes, so it carries real names and titles. The schema contract lives in the Brief repo's `LINKED_APPS.md`; changes must stay additive.
