...
- Conversations logged *before* any of this existed are read too. Past Events and relationship inference both run inside the capture pipeline, so on their own they would only ever see captures processed after the update, leaving the feed empty on a database full of history. A one-time backfill sweeps the existing timeline (imported Instagram threads, voice notes, typed captures) and applies the same two extractions. It is offline-only, so a retroactive pass never spends your AI budget, and it de-duplicates, so it can't double anything. Settings → Data → "Rescan Past Conversations" runs it again on demand, e.g. after restoring an old backup.
